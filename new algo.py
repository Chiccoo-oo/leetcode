...
class MazeSolver(Node):
    ...
    def control_loop(self):
        ...
        # If robot has reached Area 3 (end), stop
        ...
        # If robot is at Area 1 (start) and an obstacle is detected, stop
        ...
        # New algorithm: Obstacle avoidance and movement strategy
        if self.front_distance < 0.5:  # Obstacle in front
            if self.left_distance > self.right_distance:  # Prefer turning left
                twist_msg.linear.x = 0.0
                twist_msg.angular.z = 0.5  # Turn left
            else:  # Prefer turning right
                twist_msg.linear.x = 0.0
                twist_msg.angular.z = -0.5  # Turn right
        elif self.left_distance < 0.5:  # Obstacle on the left
            twist_msg.linear.x = 0.2
            twist_msg.angular.z = -0.5  # Turn right to avoid left obstacle
        elif self.right_distance < 0.5:  # Obstacle on the right
            twist_msg.linear.x = 0.2
            twist_msg.angular.z = 0.5  # Turn left to avoid right obstacle
